...
prop_Expr :: Expr -> Bool
prop_Expr (Num n) | n >= 0 = True
                  | otherwise = False
prop_Expr (Operation AddOp a b) = prop_Expr a && prop_Expr b
prop_Expr (Operation MulOp a b) = prop_Expr a && prop_Expr b
...
showExpr (Operation AddOp a b) = showExpr a ++ " + " ++ showExpr b
showExpr (Operation MulOp a b) = showExpr a ++ " * " ++ showExpr b
showExpr (Pow n) = "x^" ++ show n
...
eval :: Int -> Expr -> Int
eval x (Num n) = n
eval x (Operation AddOp a b) = (eval x a) + (eval x b)
eval x (Operation MulOp a b) = (eval x a) * (eval x b)
eval x (Pow n) = x^n
